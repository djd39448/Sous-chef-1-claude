...
struct HomeScreen: View {
    ...
    /// user into chat to type the request out. The backend kicks off
    /// image generation in the background; `pollForFreshPlan` keeps
    /// re-fetching the week until every day has a photo.
    @MainActor
    private func generatePlan() async {
        ...
        do {
            ...
            await load()
            await pollForFreshPlan(client: client)
        } catch let e as APIError where e.isBenignCancellation {
            return
        } catch {
    ...
    /// Drain the image-poll stream for the user's current week, patching
    /// the loaded state's `plan` slot on each yield so photos appear as
    /// the backend persists them. Profile + ingredients stay untouched.
    @MainActor
    private func pollForFreshPlan(client: APIClient) async {
        let week = DateUtil.todaysMondayString()
        for await fresh in client.pollForReadyImages(weekStart: week) {
            if case .loaded(let profile, _, let ingredients) = loadState {
                loadState = .loaded(profile: profile, plan: fresh, ingredients: ingredients)
            }
        }
    }
